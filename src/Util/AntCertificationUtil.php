...
class AntCertificationUtil
{
     ...
     * 验证支付宝公钥证书是否可信
     *
     * @param $alipayCert string 支付宝公钥证书
     * @param $rootCert  string  支付宝根证书
     *
     * @return bool 验证证书是否可信
     */
    public function isTrusted($alipayCert, $rootCert)
    {
     ...
     * @return null|bool
     */
    public function verifySignature($alipayCert, $rootCert)
    {
        ...
        if (!$checkSign) {
            ...
            for ($i = 0; $i < $length; $i++) {
                ...
                if ($issuer == $subject) {
                    return $this->isCertSigner($alipayCertArray[1] . '-----END CERTIFICATE-----', $rootCertArray[$i] . '-----END CERTIFICATE-----');
                }
            }
     ...
     * @return bool
     */
    public function verifyCertChain($alipayCerts, $rootCerts)
    {
     ...
     * @return bool
     */
    public function sortByDn(&$certs)
    {
        //是否包含自签名证书
     ...
     * @return bool
     */
    public function isSelfSigned($cert)
    {
     ...
     * @param $current string   当前需要插入证书链的证书，include
     */
    public function addressingUp($subjectMap, &$certChain, $current)
    {
        $certChain[] = $current;
     ...
     * @return mixed
     */
    public function addressingDown($issuerMap, &$certChain, $current)
    {
     ...
     * @param null|mixed $certPem
     * @param null|mixed $caCertPem
     *
     * @return bool
     */
    public function isCertSigner($certPem = null, $caCertPem = null)
    {
